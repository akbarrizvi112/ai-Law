...
export class UnstructuredLoader extends BaseDocumentLoader {
  ...
  async _partition() {
    ...
    formData.append("files", new Blob([buffer], { type: "application/pdf" }));
...
export class UnstructuredDirectoryLoader extends DirectoryLoader {
  constructor(
    ...
    const loaders = UNSTRUCTURED_API_FILETYPES.reduce((loadersObject: LoadersMapping, filetype: string) => {
