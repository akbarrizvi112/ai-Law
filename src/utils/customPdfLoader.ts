...
abstract class BaseDocumentLoader implements DocumentLoader {
  loadAndSplit(textSplitter?: TextSplitter | undefined): Promise<Document<Record<string, any>>[]> {
...
export class WebPDFLoader extends BaseDocumentLoader {
  ...
  constructor(blob: Blob, { splitPages = true, pdfjs = PDFLoaderImports, parsedItemSeparator = "" } = {}) {
    super();
    this.blob = blob;
